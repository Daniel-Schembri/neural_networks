...
\Begin
{
    ...
    {
        \tcp{Done for each}
        Set network inputs $y(0), x(0), x(1), ..., x(k)$\\
        Forward-propagate.\\ 
        Error $e = target - y(t+k)$\\
        Backpropagate the error across the network.\\
        Update all the weights in the network according to standard Backpropagation.\\
        Average the weights of the unfolded network that correspond to $g$ in the RNN.\\
        Set each of those weights to the averaged $g$ value\\
    }
}
