...
They also lack the surprising robustness of biological nerve networks, since they rely on a flawless operation of all components. Nevertheless, 
...
BSB = Brain-State-in-a-Box, ART = Adaptive Resonance Theory, SRN = Simple recurrent network, LTSM = Long short term memory}
\label{fig:class}
\end{figure}
...
Consider the network shown in figure \ref{fig:1on1}. The activation function of the output neuron is chosen to be the sigmoid function. 
...
%{%
%  \label{fig:roul}%
%  \caption{The probability distribution during the selection process. The size of each slice corresponds
%           to the score (as written inside each slice) of the corresponding agent.}%
%}
%\capbtabbox{%
%  \begin{tabular}{|c|c|} \hline
%     Agent & Score \\ \hline
...
%}{%
%%  \label{tab:rank}%
%  \caption{An example fitness ranking of 5-Agent population. The score corresponds to the objects found.}%
%}
%\end{floatrow}
%\end{figure}
