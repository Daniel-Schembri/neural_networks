...
\begin{acronym}
    ...
    \acro{$a_j$}{Activity of unit $j$}
    \acro{$a_j(t)$}{Activity of unit $j$ at time $t$}
    ...
    \acro{$w_{ij}$}{Weight on the connection link between unit $i$ and $j$}
...
An appraisal of advantages and detriments of the presented models is rahter difficult as there performance hugly depends on the specific application. The selection and appropriate usage of network types, as well as the chosen sturcutre requires extensive experience. 
...
In order to allow computers quick and easy calculations, weights are often stored in matrices, representing the whole model as a basic mathematical unit. Let $N_{i}$ be a neural network, consisting of $i$ layers: Input,  output and $i-2$ hidden layers. $N_{i}$ can be fully described by $i-1$ matrices $W_{ij}$ where $i$ is the number of units in the preceding layer, and $j$ the amount of neurons that are connected to any of those $i$ units:
\begin{equation}
W = 
\begin{pmatrix}
w_{11} & \cdots & w_{i1} \\
