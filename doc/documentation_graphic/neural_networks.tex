...
In general an ANN has an inputlayer, containing the inputvector $\vec{X}$, optional hidden layers and one outputlayer.
All layers have real weights, thresholds and output values.
...
\end{tikzpicture}
	\caption{The general structure of an artificial neural net}
...
\begin{figure}[H]
	\begin{center}
		...
		\path (I-1) edge node[above]{$w_{1,1}$} (H-1);	
		\path (I-2) edge node[above]{$w_{1,2}$} (H-1);
		\path (I-3) edge node[above]{$w_{1,3}$} (H-1);					
		\end{tikzpicture}
...
In this section the different neuron- and networktypes are described.
