...
The output of the neuron is then computed by an activation function $f(I)$. If the computing neuron is a hidden neuron the output is also weighted and passed to the next unit. If it is an output neuron then the output is a component of the Outputvector $Y$ Usually all neurons of an entire layer have the same activation function. Commonly used are tangens hyperbolicus and linear functions:
...
\begin{figure}[H]
	\begin{subfigure}[]{0.5\linewidth}
		...
		\addplot[domain=0:3,mark=none,draw=black,ultra thick] {1.2};
		\end{axis}
		
		\draw plot[mark=*, mark options={fill=white}] (3.8,3.8);
		\draw plot[mark=*, mark options={fill=white}] (3.5,3);
		\draw plot[mark=*, mark options={fill=white}] (4.5,3.6);
		
		\draw plot[mark=+, mark options={fill=white}] (5.7,2.6);
		\draw plot[mark=+, mark options={fill=white}] (4.5,2);
		\draw plot[mark=+, mark options={fill=white}] (5.3,2.3);
	...
	\begin{tikzpicture}		
		...
		\draw plot[mark=*, mark options={fill=white}] (3.5,2.7);
		\draw plot[mark=*, mark options={fill=white}] (4.5,3.6);
	...
	\hspace{0.5cm}
		\begin{subfigure}[]{0.5\linewidth}
			...
			\addplot[domain=0:3,mark=none,draw=black,ultra thick] {abs(0.5*\x))};
			\end{axis}
